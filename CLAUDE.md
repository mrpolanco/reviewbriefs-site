...
reviewbriefs-site/
  ...
  prompts/
    weekly-review-brief.md   ← Claude prompt used to generate reports manually
...
**Layer 2 — Analysis object:** Structured JSON output (`data/brief-schema.json` defines the shape). Generated by `prompts/packet-to-analysis.md`. Feeds both briefs.

**Layer 3 — Presentation:** Two HTML files per job, rendered from the analysis object.

### Prompts (run in Claude Code session)
